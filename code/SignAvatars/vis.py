...
text_dict = {}
hamnosys_path = os.path.join('datasets', 'hamnosys2motion', 'data.json')
if os.path.exists(hamnosys_path):
    with open(hamnosys_path, 'rb') as f:
        hamnosys_text_list = json.load(f)
    for i in hamnosys_text_list.keys():
        text_dict[i] = hamnosys_text_list[i]['hamnosys_text']

csv_path = os.path.join('datasets', 'language2motion', 'text', 'how2sign_realigned_train.csv')
if os.path.exists(csv_path):
    text_all = pd.read_csv(csv_path, sep='\t', names=["VIDEO_ID", "VIDEO_NAME", "SENTENCE_ID", "SENTENCE_NAME", "START_REALIGNED","END_REALIGNED","SENTENCE"])
...
bg_path = os.path.join('assets', 'blender.png')
background = cv2.imread(bg_path) if os.path.exists(bg_path) else np.zeros((720, 1280, 3), dtype=np.uint8)
org = (10, 30)
...
def process_single_video(file_name, result_path):
    ...
    img_list = []
    text = text_dict.get(video_name.replace('/', ''), '')
