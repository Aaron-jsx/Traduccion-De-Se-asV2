...
def quat2mat(quat):
    """
    将四元数转换为旋转矩阵
    :param quat: 四元数数组，形状为 (N, 4)，其中第一个元素为实部，后三个元素为虚部
    :return: 旋转矩阵数组，形状为 (N, 3, 3)
    """
    # 归一化四元数
...
if __name__ == "__main__":
    ...
    video_id = '_93xgBE3NgA_6-5-rgb_front'
    pkl_dir = r"/path/to/how2sign_pkls"
    data = joblib.load(open(os.path.join(pkl_dir, f"{video_id}.pkl"), "rb"))
    ...
    # load imgs
    video_path = os.path.join('/path/to/videos', f'{video_id}.mp4')
